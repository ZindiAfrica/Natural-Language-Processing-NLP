...
all augmentation , with sampler, efficinetB7
"""
...
class SpecAugmentation(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.replace == "zero":
          ...
          replacement = input.mean(-1).median(-1)[0]
...
class AudioGeneratorDataset(torch.utils.data.Dataset):
    def __init__(self, path_audio, y, resample_freq = 32000, max_length=3, augmentation=[], validation=False, num_class=264, ):
      ...
      'watermelon': 190, 'weeding': 191, 'worm': 192}
        ...
        self.y = np.array([self.labels2idx[t] for t in y]).astype(np.int64) # name class to int
    ...
    def __getitem__(self, index):
        ...
        if "noise" in self.augmentation:
                ...
                #signal_raw = AddGaussianNoisev2(signal_raw)
...
class Config():
    def __init__(self):
        ...
        self.kfold=5
        self.name = "model_7"
        self.save_name = f"{self.name}-GIZ-{self.kfold}kfolds" #Cnn14_DecisionLevelAtt-GIZ.pth" ResNet38-GIZ
...
# activate data augmentation from audiomentation
augmentations = ["noise"]
...
# change path where samples are stored
...
# loss function
loss_fnt =nn.BCEWithLogitsLoss(reduction="none")  
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(paths)), targets_names)):
        ...
        backbone = torch.hub.load('rwightman/gen-efficientnet-pytorch', "tf_efficientnet_b6_ns", pretrained=True)
 
        model = AudioClassifierHub(backbone, cfg.resample_freq, window_size, hop_size, mel_bins, fmin, fmax, cfg.num_class).to(cfg.device)
