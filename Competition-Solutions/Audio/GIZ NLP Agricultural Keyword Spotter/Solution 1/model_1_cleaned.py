...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(paths)), targets_names)):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, amsgrad=False) # torch.optim.SGD(model.parameters(), lr=1e-3, momentum=5e-4, nesterov=True)# 
