...
parser.add_str('--model_dir_or_name', default='Davlan/mt5_base_yor_eng_mt', help='Model name or directory containing weights and vocab')
parser.add_int('--max_len',           default=256,           help='Maximum length of input sequence (training phase)')
...
parser.add_int('--seed',              default=188328,        help='Random seed global')
...
parser.add_int('--n_examples_train_total',  default=10054,   help='Number of training examples. This value is used to define an epoch')
parser.add_int('--n_epochs',          default=25,            help='Number of epochs to train')
parser.add_int('--batch_size',        default=1,             help='Batch size')
parser.add_int('--accumulation_steps', default=32,           help='Accumulation steps')
parser.add_int('--batch_size_infer',  default=16,            help='Batch size for inference')
...
print(train_df['English'].iloc[0]) # A rather small thing: this is enough for me.
...
encoded_dict_yoruba_train = tokenizer.batch_encode_plus(
    list(train_df['Yoruba'].values),
...
encoded_dict_english_train = tokenizer.batch_encode_plus(
    list(train_df['English'].values),
    max_length=args.max_len,
    add_special_tokens=True,
    truncation=True,
    padding='max_length',
...
encoded_dict_yoruba_test = tokenizer.batch_encode_plus(
    list(test_df['Yoruba'].values),
    max_length=args.max_len,
    add_special_tokens=True,
    truncation=True,
    padding='max_length',
...
X = np.array(encoded_dict_yoruba_train['input_ids'], dtype=np.int32)
Y = np.array(encoded_dict_english_train['input_ids'], dtype=np.int32)
X_test = np.array(encoded_dict_yoruba_test['input_ids'], dtype=np.int32)
...
if args.job != 'score' and args.job != 'subm' and args.job != 'score_subm':
    for fold_id in range(args.initial_fold, args.final_fold):
        ...
        if 'train' in args.job:
            model = tr.MT5ForConditionalGeneration.from_pretrained(args.model_dir_or_name)
        else:
        ...
        if 'train' in args.job and not args.skip_training:
            ...
            for epoch in range(args.n_epochs):
                ...
                if mean_rouge >= best_score:
                    print('Saving model (%.6f is better than %.6f)...' % (mean_rouge, best_score))
                    best_score = mean_rouge
                    # p = os.path.join(args.out_dir, 'model-best-f%d.bin' % fold_id)
                    p = os.path.join(args.out_dir, 'model-best-f%d-e%03d-%.4f.bin' % (fold_id, epoch, mean_rouge))
                    torch.save(model.state_dict(), p)
                    print('Saved model:', p)
                else:
                    print('Mean ROUGE is not better (%.6f is not better than %.6f): NOT saving the model' % (mean_rouge, best_score))
                #
        ...
        for tta_id in range(args.tta_number + 1):
            ...
            if 'test' in args.job:
                ...
                with open(os.path.join(args.data_preds_dir, 'ids_pred_test_fold_%d_tta_%d.pkl' % (fold_id, tta_id)), 'wb') as f:   
                    pickle.dump(preds, f)
                #
                test_df['English'] = np.concatenate(text_pred)
                # test_df[['ID', 'English']].to_csv('text_pred_test_fold_%d_tta_%d.csv' % (fold_id, tta_id), index=False)
                test_df[['ID', 'English']].to_csv('../../submission-final.csv', index=False, encoding='utf-8')
        #--------------------------------------------------------------------------
        print('Cleaning...')
        try:
            del model
